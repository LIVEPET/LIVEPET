...
const plans = [
  {
    ...
    perks: [
      {
        icon: Heart,
        title: "1 pet cadastrado",
        text: "Crie o perfil completo do seu melhor amigo.",
      },
      {
        icon: Bell,
        title: "Lembretes básicos",
        text: "Notificações de vacina e vermífugo.",
      },
      {
        icon: ShieldCheck,
        title: "Carteira digital",
        text: "Guarde documentos essenciais no celular.",
      },
      {
        icon: Headphones,
        title: "Suporte por e-mail",
        text: "Respondemos em até 48h úteis.",
      },
    ],
  },
  {
    ...
    badge: "Mais popular",
    tagline:
      "O plano que 8 em cada 10 tutores escolhem para cuidar com tudo no lugar.",
    perks: [
      {
        icon: Heart,
        title: "Até 5 pets",
        text: "Cadastre toda a família peluda em uma só conta.",
      },
      {
        icon: Bell,
        title: "Lembretes inteligentes",
        text: "IA sugere a melhor data e horário para cada cuidado.",
      },
      {
        icon: Crown,
        title: "Pedigree validado",
        text: "Árvore genealógica oficial em até 24h.",
      },
      {
        icon: Stethoscope,
        title: "Agenda com vets parceiros",
        text: "Marque consultas com até 30% de desconto.",
      },
      {
        icon: MapPin,
        title: "Rastreamento em tempo real",
        text: "Saiba onde seu pet está, mesmo no passeio.",
      },
      {
        icon: HeartHandshake,
        title: "Parcerias premium",
        text: "Cupons exclusivos com vets, petshops e estética.",
      },
      {
        icon: Dna,
        title: "Compatibilidade genética",
        text: "Análise para acasalamentos seguros.",
      },
      {
        icon: Headphones,
        title: "Suporte 24h",
        text: "Atendimento prioritário por chat e telefone.",
      },
    ],
  },
  {
    ...
    perks: [
      {
        ...
        text: "Cadastre quantos animais quiser, sem limite.",
      },
      {
        icon: Users,
        title: "Até 5 tutores",
        text: "Toda a família acessa e atualiza juntos.",
      },
      {
        icon: Crown,
        title: "Pedigree + 4 gerações",
        text: "Árvore genealógica completa e exportável.",
      },
      {
        icon: Stethoscope,
        title: "Histórico veterinário",
        text: "Linha do tempo médica de cada pet.",
      },
      {
        icon: CloudUpload,
        title: "Backup em nuvem",
        text: "Tudo seguro, com criptografia de ponta.",
      },
      {
        icon: HeartHandshake,
        title: "Parcerias prioritárias",
        text: "Acesso antecipado a novos cupons e parceiros.",
      },
      {
        icon: Zap,
        title: "API & exportação",
        text: "Integre com sistemas de canil e clínicas.",
      },
      {
        icon: Headphones,
        title: "Gerente dedicado",
        text: "Um humano sempre disponível pra você.",
      },
    ],
  },
];
...
const Pricing = () => {
  ...
  return (
    <section
      id="plans"
      className="relative overflow-hidden gradient-soft py-24"
    >
      ...
      <div className="container relative">
        <div className="mx-auto mb-14 max-w-2xl text-center">
          ...
          <h2 className="mt-3 font-display text-4xl font-bold leading-tight text-foreground text-balance sm:text-5xl">
            Escolha o que <span className="italic text-primary">combina</span>{" "}
            com você
          </h2>
        ...
        <div className="grid gap-6 md:grid-cols-3">
          {plans.map((p) => (
            ...
            >
              ...
              <h3
                className={`font-display text-2xl font-bold ${p.highlight ? "text-primary-foreground" : "text-foreground"}`}
              >
                {p.name}
              </h3>
              <p
              ...
              <div className="mt-6 flex items-end gap-1">
                <span
                  className={`text-sm font-semibold ${p.highlight ? "text-primary-foreground/80" : "text-muted-foreground"}`}
                >
                  R$
                </span>
                <span
                  className={`font-display text-5xl font-bold ${p.highlight ? "text-primary-foreground" : "text-foreground"}`}
                >
                  {p.price}
                </span>
                <span
                  className={`mb-2 text-sm ${p.highlight ? "text-primary-foreground/70" : "text-muted-foreground"}`}
                >
                  {p.period}
                </span>
              </div>
              ...
              <ul className="my-7 space-y-3">
                {p.features.map((f) => (
                  <li key={f} className="flex items-start gap-3 text-sm">
                    <span
                      className={`mt-0.5 flex h-5 w-5 shrink-0 items-center justify-center rounded-full ${p.highlight ? "bg-primary-foreground/20" : "bg-primary-soft"}`}
                    >
                      <Check
                        className={`h-3 w-3 ${p.highlight ? "text-primary-foreground" : "text-primary"}`}
                        strokeWidth={3}
                      />
                    </span>
                    <span
