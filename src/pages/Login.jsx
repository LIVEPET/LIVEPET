...
const Login = () => {
  ...
  return (
    <div className="relative min-h-screen overflow-hidden bg-gradient-to-br from-secondary via-background to-primary-soft">
      ...
      <div
        className="blob h-[460px] w-[460px] -right-32 bottom-0 bg-accent-warm/25 animate-blob"
        ...
        <section className="hidden flex-col gap-8 lg:flex">
          <div>
            <span className="inline-flex w-fit items-center gap-2 rounded-full border border-primary/20 bg-primary-soft px-4 py-2 text-xs font-semibold text-primary shadow-soft">
              <Sparkles className="h-3.5 w-3.5" />A vida do seu pet, organizada
              de verdade
            </span>
            <h1 className="mt-6 font-display text-5xl font-bold leading-[1.05] text-foreground">
              Mais carinho.{" "}
              <span className="italic text-primary">Menos planilhas.</span>
            </h1>
            <p className="mt-5 max-w-md text-base text-muted-foreground">
              Prontuário, vacinas, agendamentos e pedigree em um app feito com —
              e para — quem ama animais.
            </p>
          </div>
            ...
            {[
              {
                ...
                desc: "Histórico completo sempre na mão.",
              },
              {
                icon: Bell,
                title: "Lembretes inteligentes",
                desc: "Vacinas e consultas no tempo certo.",
              },
              {
                icon: Shield,
                title: "Validação de pedigree",
                desc: "Disponível nos planos Pro e Família.",
              },
            ].map(({ icon: Icon, title, desc }) => (
              <li
                key={title}
                className="group flex items-start gap-4 rounded-2xl border border-border/60 bg-card/80 p-4 backdrop-blur shadow-soft"
              >
